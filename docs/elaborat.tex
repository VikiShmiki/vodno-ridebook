...
and installs the pinned requirements. Stage two starts from the same slim
base, copies only the finished virtualenv and the \cd{app} package, and never
sees pip's caches or build tooling. A dedicated user is created and the image
ends with \cd{USER vodno} (uid 1001), so no application code runs as root. A
\cd{HEALTHCHECK} calls \cd{/api/health} using the standard library, avoiding an
extra \cd{curl} dependency.

\subsection{Frontend Dockerfile}

Stage one, on \cd{node:24-alpine}, copies \cd{package.json} and
\cd{package-lock.json} first and runs \cd{npm ci} before the source is copied,
so the dependency layer is reused whenever only application code changes. It
then runs \cd{npm run build}. Stage two is
...
Two details are worth calling out. First, the backend suite runs against a
...
merge. Nothing is published from a pull request.
...
\cd{GITHUB\_TOKEN}, which GitHub Actions provides automatically and which
authenticates the push to GHCR, and the optional \cd{KUBE\_CONFIG} repository
secret holding a base64-encoded kubeconfig for the deploy job.

Because the \cd{secrets} context is not available in a job-level \cd{if}, the
\cd{publish} job turns the presence of \cd{KUBE\_CONFIG} into a job output,
and the \cd{deploy} job keys off that. When no cluster is configured the
deploy job is skipped and the pipeline still succeeds.
...
capabilities dropped and the \cd{RuntimeDefault} seccomp profile; the writable
paths each service genuinely needs are \cd{emptyDir} volumes.
...
\begin{itemize}
  ...
  \item A CI pipeline: linting, type checking, tests against a real
        ...
        builds and a full Compose smoke test --- and a CD pipeline re-using
        that gate, publishing to GHCR under \cd{latest} and commit-SHA tags
        and rolling the Deployments.
