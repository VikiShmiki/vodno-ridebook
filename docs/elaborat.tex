...
deploy on a self-hosted runner sitting next to the cluster
  render full-SHA manifests → kubectl apply → rollout status → Ingress smoke test
...
\begin{itemize}
  ...
  \item \textbf{Argo CD} --- moving to GitOps would commit release image
        digests instead of rendering them in the deployment workflow.
